...
public class GameUtility
{
    ...
    public static float GetValueWithEvent(Event @event, float time)
    {
        ...
        if (time > @event.endTime && @event.endValue == 0) return -.0001f;
    ...
    private static void VerifyWhetherOrNotNegation(List<Keyframe> keyframes, int index_firstKeyframe, int index_lastKeyframe, ref Keyframe firstKeyframe, ref Keyframe lastKeyframe, ref int sign_inTangent_firstKeyframe, ref int sign_outTangent_lastKeyframe)
    {
        if (index_lastKeyframe + 1 >= keyframes.Count)//如果已经是最后一个，直接跳过下一个点的正负性检测
        {
            goto SkipLastKeyframe;
        }
        //检测下一个点是否需要取反，原理：如果Value>0并且斜率小于0并且不是同一时间上的Key，需要再次取反
        if (lastKeyframe.value > keyframes[index_lastKeyframe + 1].value
            && lastKeyframe.time != keyframes[index_lastKeyframe + 1].time
            && lastKeyframe.value > 0)
        {
            NegationValue(ref sign_outTangent_lastKeyframe);//取反操作
        }
    SkipLastKeyframe:
        //检测上一个点是否需要取反，原理：如果Value>0并且斜率小于0并且不是同一时间上的Key，需要再次取反
        if (firstKeyframe.value < keyframes[index_firstKeyframe - 1].value
            && firstKeyframe.time != keyframes[index_firstKeyframe - 1].time
            && firstKeyframe.value < 0)
        {
            NegationValue(ref sign_inTangent_firstKeyframe); //取反操作
        }
    }
    /// <summary>
    /// 对数值取反
    ...
    private static void DisposeKey(Blophy.Chart.Event[] speeds, List<Keyframe> keys, Vector2 keySeed, int i, float tant)
    {
    ...
    private static Keyframe InstKeyframe(Blophy.Chart.Event[] speeds, Vector2 keySeed, int i, float tant, int index)
    {
